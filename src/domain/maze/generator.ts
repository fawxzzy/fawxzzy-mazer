...
const rasterizeMaze = (options: RasterizeOptions): MazeBuildResult => {
  const { core, seed, shortcutsCreated } = options;
  const scale = (core.width * 2) - 1;
  const tiles = createGrid(scale).map(resetTile);
  ...
  for (let y = 0; y < core.height; y += 1) {
    for (let x = 0; x < core.width; x += 1) {
      ...
      const centerIndex = indexFromCoordinates(center.x, center.y, scale);
      ...
      for (const dir of DIRS) {
        ...
        const passageIndex = indexFromCoordinates(passageX, passageY, scale);
  ...
  const startIndex = indexFromCoordinates(startPoint.x, startPoint.y, scale);
  const endIndex = indexFromCoordinates(goalPoint.x, goalPoint.y, scale);
  const solution = solveTileAStar(tiles, scale, startPoint, goalPoint);
  const pathIndices = solution.path.map((point) => indexFromCoordinates(point.x, point.y, scale));

  for (const index of pathIndices) {
    tiles[index].path = true;
  }
  tiles[endIndex].end = true;

  const wallIndices = tiles
  ...
  return {
    ...
    core,
    solution,
    metrics: measureTileMaze(tiles, scale, pathIndices)
  };
};
...
const solveTileAStar = (
  ...
  const startIndex = indexFromCoordinates(start.x, start.y, scale);
  const goalIndex = indexFromCoordinates(goal.x, goal.y, scale);
  ...
  while (open.size > 0) {
    ...
    if (current.index === goalIndex) {
      return {
        found: true,
        path: reconstructTilePath(cameFrom, current.index, scale),
        visited,
        expanded,
    ...
    for (const next of openFloorNeighbors(tiles, current.index)) {
      ...
      const point = pointFromIndex(next, scale);
      open.push({
        index: next,
        g: tentativeG,
...
const measureTileMaze = (tiles: MazeTile[], scale: number, pathIndices: number[]): MazeMetrics => {
  let deadEnds = 0;
  let junctions = 0;
  ...
  for (let index = 1; index < pathIndices.length - 1; index += 1) {
    const a = pointFromIndex(pathIndices[index - 1], scale);
    const b = pointFromIndex(pathIndices[index], scale);
    const c = pointFromIndex(pathIndices[index + 1], scale);
    const abx = b.x - a.x;
    const aby = b.y - a.y;
    const bcx = c.x - b.x;
    const bcy = c.y - b.y;
...
const reconstructTilePath = (cameFrom: number[], endIndex: number, scale: number): Point[] => {
  const path: Point[] = [];
  let cursor = endIndex;

  while (cursor >= 0) {
    path.push(pointFromIndex(cursor, scale));
...
const pointFromIndex = (index: number, scale: number): Point => ({
  x: index % scale,
  y: Math.floor(index / scale)
});
