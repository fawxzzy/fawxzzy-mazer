...
1. Latest repo-owned visual artifacts from the screenshot gate in `tmp/captures/mazer-visual/`
2. The current visual assertions in `tests/scenes/demo-build.test.ts`
3. Current runtime/tooling in `scripts/visual/capture.mjs` and `scripts/visual/gate.mjs`
